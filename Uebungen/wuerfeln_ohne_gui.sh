...
runde() {
  ...
  for i in {1..3}; do
    ...
    echo -e "$GREEN Wurfwert: $groesser * 10 + $kleiner = $((groesser * 10 + kleiner))$RESET"
...
spiel_starten() {
  ...
  echo -e "$GREEN Dein bestes Ergebnis: $punkt_spieler$RESET"
  echo -e "$RED Computers bestes Ergebnis: $punkt_computer$RESET"

  if [[ $punkt_spieler -gt $punkt_computer ]]; then
    echo -e "\n$GREEN Du hast gewonnen! Herzlichen Glückwunsch, $benutzername!$RESET"
  elif [[ $punkt_spieler -lt $punkt_computer ]]; then
    echo -e "\n$RED Der Computer hat gewonnen! Versuch es beim nächsten Mal!$RESET"
  else
    echo -e "\n$YELLOW Es ist ein Unentschieden! Gut gespielt!$RESET"
  fi
}
