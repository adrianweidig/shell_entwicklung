...
# Prüfen, ob der Benutzer zur `cron`-Gruppe gehört
if ! groups $(whoami) | grep -q "\bcron\b"; then
    echo "${RED}WARNUNG: Sie sind nicht Mitglied der Gruppe 'cron', daher können keine Cron-Aufgaben hinzugefügt werden.${RESET}"
    echo "Bitte fügen Sie sich der Gruppe 'cron' hinzu, um Cron-Aufgaben zu planen."
    exit 1
fi
...
# Prüfen, ob der Benutzer zur `at`-Gruppe gehört
if ! groups $(whoami) | grep -q "\bat\b"; then
    echo "${RED}WARNUNG: Sie sind nicht Mitglied der Gruppe 'at', daher können keine At-Aufgaben hinzugefügt werden.${RESET}"
    echo "Bitte fügen Sie sich der Gruppe 'at' hinzu, um At-Aufgaben zu planen."
    exit 1
fi
